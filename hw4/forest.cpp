...
void buildtree(std::vector<double *> &data, int total , int level)
{
if(total == 1){
...
level++;
buildtree(dataup, count, level);
for(int i = 0; i < level - 1; i++)   //!!!
	cout<<"\t";
cout<<"else {\n";
buildtree(datadown, (total - count), level);
level--;
for(int i = 0; i < level - 1; i++)   //!!!
	cout<<"\t";
if(level == 1)
...
for(int i = 1; i <= tree; i++){
	cout<< "tree"<< i<< "_predict:\n";
	std::vector<double*> newdata;
	std::random_shuffle ( data.begin(), data.end());
	for(int i = 0; i < total / 3; i++)
		newdata.push_back(data[i]);
	buildtree(newdata, total / 3, level);
	}
cout<<"voting:\n" << "\tif(t > f)	return 1;\n" << "\telse	return -1;\n" << "}\n";
return 0;
}
